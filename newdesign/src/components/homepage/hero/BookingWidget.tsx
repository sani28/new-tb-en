...
export default function BookingWidget() {
  ...
  /* Book action — always opens the promo checkout flow */
  const onBook = () => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 items-center gap-2 md:basis-auto md:flex-1 md:gap-3 max-[400px]:gap-1.5">
        ...
        <div className="relative flex-1 min-w-0 md:min-w-[220px] z-[var(--z-dropdown)]" ref={calendarRef}>
          ...
          {calendarOpen && (
            <div
              className={
                "absolute bottom-[calc(100%+8px)] left-0 w-[350px] bg-white rounded-2xl p-5 " +
                "shadow-[0_8px_32px_rgba(0,0,0,0.15),0_2px_8px_rgba(0,0,0,0.1)] z-[var(--z-dropdown)] " +
                "max-md:fixed max-md:bottom-auto max-md:top-1/2 max-md:left-1/2 max-md:-translate-x-1/2 max-md:-translate-y-1/2 " +
                "max-md:w-[90%] max-md:max-w-[350px] max-md:max-h-[90vh] max-md:overflow-y-auto max-md:z-[var(--z-modal)]"
              }
              role="dialog"
      ...
      <button
        className={
          "basis-full rounded-[10px] py-3 text-[14px] bg-[var(--color-brand-red)] " +
          "font-[var(--font-copperplate)] font-bold uppercase tracking-[0.3px] text-white transition-colors " +
          "hover:bg-[#C4001C] " +
          "md:basis-auto md:flex-none md:shrink-0 md:min-w-[140px] md:self-stretch md:px-8 md:py-3 md:text-[15px] md:tracking-[0.2px]"
        }
        onClick={onBook}
