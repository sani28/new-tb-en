...
import { usePromoCheckout, TOUR_META, TOUR_PRICES } from "./checkout/PromoCheckoutContext";
  ...
  return (
    <div className="relative h-full">
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        {details && (
          <>
            <p className="mb-2 text-[14px] text-[#666] leading-[1.4]">{details.description}</p>
            <div className="mb-2 flex gap-3 text-[14px] text-[#999]">
              <span>{details.stops} stops</span>
              <span>{details.durationHours}h route</span>
            </div>
            <ul className="mb-2.5 space-y-1.5">
              {details.highlights.map((h) => (
                <li key={h} className="flex items-center gap-1.5 text-[14px] text-text-dark">
        ...
        <div className="mt-auto border-t border-[#eee] pt-2">
          <div className="mb-1.5 flex items-baseline justify-between">
            <span className="text-[12px] text-[#666]">Adult</span>
            <div className="flex items-baseline gap-1.5">
              <span className="text-[11px] text-[#999] italic">from</span>
              <span className="text-[10px] text-[#999] line-through">${prices.adultOrig.toFixed(2)}</span>
              <span className="text-[14px] font-bold text-[#FF0000]">${prices.adult.toFixed(2)}</span>
            </div>
          </div>
          <div className="mb-3 flex items-baseline justify-between">
            <span className="text-[12px] text-[#666]">Child</span>
            <div className="flex items-baseline gap-1.5">
              <span className="text-[11px] text-[#999] italic">from</span>
              <span className="text-[10px] text-[#999] line-through">${prices.childOrig.toFixed(2)}</span>
              <span className="text-[14px] font-bold text-[#FF0000]">${prices.child.toFixed(2)}</span>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            >
              Book <span className="text-[14px]">&rarr;</span>
            </button>
          </div>
        </div>
      </div>
...
function ExclusivePackageCard({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3">
        <div className="mb-0.5 text-[18px] text-text-dark leading-tight" style={{ fontFamily: "'SUIT-Heavy', sans-serif", textWrap: "balance" }}>{pkg.name}</div>
        <div className="mb-2 text-[14px] italic text-[#666]">{pkg.tagline}</div>
        <div className="mb-2 text-[13px] text-[#999]">{pkg.baseRoute}</div>
        <ul className="mb-2.5 space-y-1.5">
        ...
        <div className="mt-auto border-t border-[#eee] pt-2">
          <div className="mb-1.5 flex items-baseline justify-between">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-[11px] text-[#999] italic">from</span>
              <span className="text-[10px] text-[#999] line-through">${pkg.pricing.adultOrig.toFixed(2)}</span>
              <span className="text-[14px] font-bold text-[#FF0000]">${pkg.pricing.adult.toFixed(2)}</span>
            </div>
          </div>
          <div className="mb-3 flex items-baseline justify-between">
            <span className="text-[12px] text-[#666]">Child</span>
            <div className="flex items-baseline gap-1.5">
              <span className="text-[11px] text-[#999] italic">from</span>
              <span className="text-[10px] text-[#999] line-through">${pkg.pricing.childOrig.toFixed(2)}</span>
              <span className="text-[14px] font-bold text-[#FF0000]">${pkg.pricing.child.toFixed(2)}</span>
            </div>
          </div>
          <div className="flex gap-2">
            <button
...
function SectionHeader({ title }: { title: string }) {
  return (
    <div className="mb-4">
      ...
      <h3 className="py-2.5 text-center text-[18px] uppercase tracking-[0.2em] text-white" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
        {title}
      </h3>
      <div className="h-px bg-white/30" />
    </div>
    ...
    <div style={{ fontFamily: "'SUIT-Bold', sans-serif" }}>
      ...
      <div className="bg-brand-cream py-10 max-md:py-6 relative">
        ...
        <div className="absolute top-4 right-4 z-50 flex overflow-hidden rounded-lg border border-black/10 shadow-sm">
          {([["A", "Marquee"], ["B", "Carousel"]] as const).map(([v, label]) => (
            <button
              key={v}
              type="button"
        ...
        <h3 className="mb-8 text-center text-[18px] uppercase tracking-[0.2em] text-black" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
      ...
      <div className="text-center mt-0 bg-[#FF0019] w-full py-10 px-0 max-md:py-[30px] max-md:px-[15px]">
        <div className="max-w-[1200px] mx-auto">
          <h3 className="text-[32px] mb-10 text-white max-md:text-2xl max-md:mb-5 max-md:text-center">Bus Features</h3>
          <div className="flex justify-center gap-[60px] max-md:justify-between max-md:gap-2">
            {[
              { img: "/imgs/bus-feature-1.png", label: "Free Audio", alt: "Free Audio Guide" },
              { img: "/imgs/bus-feature-2.png", label: "Free USB", alt: "Free USB Charger" },
              { img: "/imgs/bus-feature-3.png", label: "Double Decker", alt: "Double Decker Bus" },
              { img: "/imgs/bus-feature-4.png", label: "Single Decker", alt: "Single Decker Bus" },
              { img: "/imgs/bus-feature-5.png", label: "Free Wi-Fi", alt: "Free Wi-Fi" },
            ].map((f) => (
