...
import BookingCalendar from "@/app/booking/components/BookingCalendar";

/* ── Progress indicator ──────────────────────────────────────────────────────── */
type StepStatus = "completed" | "active" | "pending";
function ProgressIndicator({ statuses }: { statuses: StepStatus[] }) {
  const labels = ["Select", "Info", "Payment"];
  return (
...
const TOUR_INFO: Record<string, { highlights: string[]; mapImage: string }> = {
  tour01: {
    highlights: [
      "Operates daily from 09:00 to 17:00 (Last bus 16:00)",
      "12 stops \u2014 hop on/off at any stop along the route",
      "Free audio guide available in 10 languages",
      "Tickets valid for 24 hours from first use",
    ],
    mapImage: "/imgs/tour01-timetable-en.png",
  },
  tour02: {
    highlights: [
      "Operates daily from 09:30 to 17:30 (Last bus 16:30)",
      "10 stops \u2014 panoramic views of Seoul skyline",
      "Free audio guide available in 10 languages",
      "Tickets valid for 24 hours from first use",
    ],
    mapImage: "/imgs/tour01-timetable-en.png",
  ...
  tour04: {
    highlights: [
      "Operates Fri\u2013Sun from 19:30 to 22:00 (Non-stop)",
      "8 illuminated landmark stops \u2014 no hop-off",
      "Free audio guide available in 10 languages",
      "Non-refundable on the day of the tour",
    ],
    mapImage: "/imgs/tour01-timetable-en.png",
  },
  "pkg-kculture": {
    highlights: [
      "Full-day curated BTS experience",
      "Includes K-Pop Museum entry pass",
      "Han River Cruise + Hanbok rental included",
      "Based on Tour 01 route",
    ],
    mapImage: "/imgs/tour01-timetable-en.png",
  },
  "pkg-kbeauty": {
    highlights: [
      "Myeongdong & Hongdae beauty districts",
      "Skin Care Workshop session included",
      "Professional style photo shoot",
      "Based on Tour 02 route",
    ],
    mapImage: "/imgs/tour01-timetable-en.png",
  },
};
...
function TourSelectionCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
    >
      <div className="relative w-full h-[120px] max-md:h-[100px] overflow-hidden">
        <img src={meta.image} alt={meta.title} className="size-full object-cover" />
        <span
          className={`absolute left-2 top-2 px-2 py-0.5 text-[10px] font-extrabold tracking-wide ${
      ...
      <div className="p-3 flex-1 flex flex-col">
        <div className={`text-[12px] font-bold leading-tight ${isActive ? "text-black" : "text-white"}`}>
          {meta.title}
        </div>

        <div className="mt-auto flex items-baseline gap-2 pt-2 border-t border-white/10">
          <div className="flex items-baseline gap-1">
            <span className={`text-[9px] ${isActive ? "text-black/30" : "text-white/30"}`}>Adult</span>
            <span className={`text-[10px] line-through ${isActive ? "text-black/25" : "text-white/25"}`}>${prices.adultOrig.toFixed(2)}</span>
            <span className="text-[13px] font-bold text-[#FF0000]">${prices.adult.toFixed(2)}</span>
          </div>
          <div className="flex items-baseline gap-1">
            <span className={`text-[9px] ${isActive ? "text-black/30" : "text-white/30"}`}>Child</span>
            <span className="text-[11px] font-bold text-[#FF0000]">${prices.child.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </button>
...
function CarouselArrows({
  ...
  return (
    <>
      <button
        type="button"
        className="absolute -left-2 top-1/2 z-10 flex size-7 -translate-y-1/2 items-center justify-center rounded-full bg-white text-black shadow-md disabled:opacity-30"
      ...
      <button
        type="button"
        className="absolute -right-2 top-1/2 z-10 flex size-7 -translate-y-1/2 items-center justify-center rounded-full bg-white text-black shadow-md disabled:opacity-30"
...
export default function PromoTourSelectionModal() {
  ...
  const selectedPrices = TOUR_PRICES[selectedTourId];
  const tourInfo = TOUR_INFO[selectedTourId];
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="popup-scrollable-content">
            ...
            <div className="bg-[#001e53] px-6 py-6 max-md:px-4">
              <h2 id="promoTourModalTitle" className="text-[20px] text-white mb-5" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
                Choose Your Tour
              </h2>

              {/* Desktop: 2-column grid */}
              <div className="hidden md:grid md:grid-cols-[1fr_auto_1fr] md:gap-4">
                {/* Exclusive packages column */}
                <div>
                  <h3 className="text-[11px] uppercase tracking-[0.15em] text-white/50 mb-3" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
                    Exclusive Packages
                  </h3>
                  <div className="relative px-3">
                    <TourSelectionCard
                      id={EXCLUSIVE_PACKAGE_IDS[exclusiveCarousel.index]}
                      isActive={selectedTourId === EXCLUSIVE_PACKAGE_IDS[exclusiveCarousel.index]}
                      onSelect={() => setSelectedTourId(EXCLUSIVE_PACKAGE_IDS[exclusiveCarousel.index])}
                    />
                    {EXCLUSIVE_PACKAGE_IDS.length > 1 && (
                      <CarouselArrows
                        onPrev={exclusiveCarousel.prev}
                        onNext={exclusiveCarousel.next}
                        prevDisabled={exclusiveCarousel.index === 0}
                        nextDisabled={exclusiveCarousel.index === EXCLUSIVE_PACKAGE_IDS.length - 1}
                      />
                    )}
                ...
                <div>
                  ...
                  <div className="relative px-3">
                    <TourSelectionCard
                      id={CLASSIC_TOUR_IDS[classicCarousel.index]}
                      isActive={selectedTourId === CLASSIC_TOUR_IDS[classicCarousel.index]}
                      onSelect={() => setSelectedTourId(CLASSIC_TOUR_IDS[classicCarousel.index])}
                    />
                    <CarouselArrows
                      onPrev={classicCarousel.prev}
                      onNext={classicCarousel.next}
                      prevDisabled={classicCarousel.index === 0}
                      nextDisabled={classicCarousel.index === CLASSIC_TOUR_IDS.length - 1}
                    />
              ...
              {/* Selected tour info — centered below columns */}
              {tourInfo && (
                <div className="border-t border-white/10 pt-4 mt-5 flex flex-col items-center text-center">
                  ...
                  <button
                    type="button"
                    className="flex items-center gap-2 px-4 py-2 border border-white/20 text-white text-[12px] hover:bg-white/10 transition-colors"
            ...
            <div className="px-6 py-6 max-md:px-4 border-b border-[#eee]">
              <h3 className="text-[14px] uppercase tracking-[0.15em] text-black mb-4" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
                Select a Date
              </h3>
            ...
            <div className="px-6 py-6 max-md:px-4 border-b border-[#eee]">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-[13px] font-bold text-black">Adult</div>
                    <div className="text-[11px] text-black/40">Ages 19\u201364</div>
                  </div>
                  <div className="flex items-center gap-3">
                    {selectedPrices && (
                      <div className="flex items-baseline gap-1 mr-2">
                        <span className="text-[10px] text-black/30 line-through">${selectedPrices.adultOrig.toFixed(2)}</span>
                        <span className="text-[14px] font-bold text-[#FF0000]">${selectedPrices.adult.toFixed(2)}</span>
                      </div>
                    )}
                    <button type="button" className="w-8 h-8 flex items-center justify-center border border-black/15 text-black/50 hover:border-black/30 transition-colors" style={{ borderRadius: "3px" }}
                      onClick={() => setAdultQty(Math.max(0, adultQty - 1))}>&#8722;</button>
                    <span className="w-6 text-center text-[14px] font-bold">{adultQty}</span>
                    <button type="button" className="w-8 h-8 flex items-center justify-center border border-black/15 text-black/50 hover:border-black/30 transition-colors" style={{ borderRadius: "3px" }}
                      onClick={() => setAdultQty(adultQty + 1)}>+</button>
                  </div>
                </div>
                {/* Child */}
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-[13px] font-bold text-black">Child</div>
                    <div className="text-[11px] text-black/40">Ages 3\u201318</div>
                  </div>
                  <div className="flex items-center gap-3">
                    {selectedPrices && (
                      <div className="flex items-baseline gap-1 mr-2">
                        <span className="text-[10px] text-black/30 line-through">${selectedPrices.childOrig.toFixed(2)}</span>
                        <span className="text-[14px] font-bold text-[#FF0000]">${selectedPrices.child.toFixed(2)}</span>
                      </div>
                    )}
                    <button type="button" className="w-8 h-8 flex items-center justify-center border border-black/15 text-black/50 hover:border-black/30 transition-colors" style={{ borderRadius: "3px" }}
                      onClick={() => setChildQty(Math.max(0, childQty - 1))}>&#8722;</button>
                    <span className="w-6 text-center text-[14px] font-bold">{childQty}</span>
                    <button type="button" className="w-8 h-8 flex items-center justify-center border border-black/15 text-black/50 hover:border-black/30 transition-colors" style={{ borderRadius: "3px" }}
                      onClick={() => setChildQty(childQty + 1)}>+</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-brand-cream px-6 py-8 max-md:px-4">
              <div className="mb-4">
                ...
                <h3 className="py-2.5 text-center text-[14px] uppercase tracking-[0.2em] text-black" style={{ fontFamily: "'SUIT-Heavy', sans-serif" }}>
          ...
          <div className="sticky bottom-0 bg-[#001e53] border-t border-white/10 py-[14px] px-[18px] shrink-0">
            ...
            >
              Continue with {TOUR_META[selectedTourId]?.title ?? "selected tour"} &rarr;
            </button>
          </div>
        </div>
      </div>
      ...
      {mapOpen && tourInfo && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 p-4" onClick={() => setMapOpen(false)}>
          <div className="relative bg-white max-w-[700px] w-full max-h-[85vh] overflow-auto" style={{ borderRadius: "4px" }} onClick={(e) => e.stopPropagation()}>
            <button type="button" className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center bg-black/60 text-white text-[18px] hover:bg-black/80 z-10" style={{ borderRadius: "2px" }}
              onClick={() => setMapOpen(false)}>&times;</button>
            <img src={tourInfo.mapImage} alt="Route Map" className="w-full h-auto" />
          </div>
        </div>
      )}
    </div>
