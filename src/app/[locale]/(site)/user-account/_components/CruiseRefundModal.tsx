...
export default function CruiseRefundModal({
  item,
  onClose,
  onSubmit,
}: {
  ...
  return (
    <RefundModal
      title="Request Refund — Cruise Experience"
      onClose={onClose}
      footer={
    ...
    >
      ...
      <div className="mb-5">
        ...
        <div className="mt-2 space-y-1 text-sm text-gray-500">
          {item.cruiseTypeName && <p>Type: {item.cruiseTypeName}</p>}
          {item.selectedDate && (
            <p>
              Date:{" "}
          ...
          {item.selectedTimeSlot && <p>Time slot: {item.selectedTimeSlot}</p>}
          {item.adultQty != null && (
            <p>
      ...
      {remaining && !remaining.passed && (
        <div className="mb-4 rounded-md bg-gray-50 p-3">
          <p className="text-sm text-gray-600">
            Time until cruise: {remaining.hours}h {remaining.minutes}m
          </p>
        </div>
      )}
      ...
      {datePassed && (
        <div className="mb-4 rounded-md border border-red-200 bg-red-50 p-3">
          <p className="text-sm font-medium text-red-700">
            This cruise date has already passed. Refunds are not available.
          </p>
        </div>
      )}
      ...
      {!datePassed && (
        <div className="mb-4 rounded-md border border-yellow-200 bg-yellow-50 p-3">
          <p className="text-sm text-yellow-800">
            This experience is operated by a third-party partner. Refund
            requests require coordination with the cruise operator and may take
            longer to process.
          </p>
        </div>
      )}
      ...
      <div className="rounded-md bg-blue-50 p-3">
        <p className="text-xs text-blue-700">
          Cruise refund requests are reviewed by our team in coordination with
          the cruise operator. Processing may take 5-7 business days.
        </p>
      </div>
    </RefundModal>
  );
